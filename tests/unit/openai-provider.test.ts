...
describe("OpenAIProvider", () => {
  it("throws a clear error when API key is missing", async () => {
    const provider = new OpenAIProvider({
      apiKey: " ",
      model,
      fetchImpl: async () => okOpenAIResponse(validOpenAIContent()),
    });

    await expect(provider.generate(makeInput())).rejects.toThrow(/requires an API key/);
  });

  it("uses official chat completions JSON mode and validates model output locally", async () => {
    let capturedUrl = "";
    let capturedInit: OpenAIFetchInit | undefined;
    const provider = new OpenAIProvider({
      ...
      fetchImpl: async (url, init) => {
        capturedUrl = url;
        capturedInit = init;
        return okOpenAIResponse(validOpenAIContent());
      },
    });
    ...
    const developerMessage = (body.messages as Array<{ role: string; content: string }>)[0]!;
  ...
  it("rejects output identity mismatches", async () => {
    ...
    await expect(provider.generate(makeInput())).rejects.toThrow(/provider must be openai/);
  });
});
...
function validBrainOutput(): Record<string, unknown> {
  return {
    ...
    taskType: "pre_market_plan",
    generatedAt,
    summary: "OpenAI mock pre-market plan.",
    structured: {
      stance: "neutral",
...
function validOpenAIContent(): string {
  return JSON.stringify(validBrainOutput());
}

function okOpenAIResponse(content: string): OpenAIFetchResponse {
  return response({
    ok: true,
    status: 200,
    text: JSON.stringify({
...
function providerWithText(text: string): OpenAIProvider {
...
function hangingFetch(): OpenAIFetchLike {
  return async (_url, init) =>
    new Promise<OpenAIFetchResponse>((_resolve, reject) => {
      const rejectAbort = () => {
